...
L'équipe elle-même a produit un audit interne daté du 2026-07-31 et un plan de
refonte (`REFACTORING_PLAN.md`) qui chiffre la dette : **86 000 lignes de
source**, **1 524 méthodes**, **1 639 lignes de baseline PHPStan** (niveau 2
sur 10 seulement), et à l'origine **2 377 erreurs PSR-12** sur 218 fichiers
(désormais résorbées en gate globale — voir §2). Ce même plan estime le
chantier de refonte restant à **90–126 jours-homme**, ce qui à lui seul
qualifie l'ampleur de la dette mieux que toute reconstruction externe.

Trois foyers de dette dominent, par impact décroissant :

1. **`site/src/Model/EditModel.php::store()`** — une méthode unique d'environ
   1 300 lignes, point de passage obligé de toute soumission d'enregistrement,
   sans aucun test comportemental, exclue de la gate PSR-12, et hébergeant deux
   `eval()` sur du texte configurable en base (`admin/forms/form.xml`,
   `filter="raw"`) qui constituent une RCE assumée par une décision de
   direction documentée.
2. **`script.php`** — 2 469 lignes / 96 méthodes dans une seule classe
   d'installation, mélangeant journalisation, permissions fichiers, migration
   de schéma, réparation de menus, gestion de plugins et purge de caches.
...
**Comportement déduit.** Le premier poste (`property.notFound`, 149 lignes,
~9 % de la baseline) est concentré dans les classes `HtmlView` des vues
(`site/src/View/PublicForms/HtmlView.php`, `site/src/View/List/HtmlView.php`…)
: elles assignent des propriétés dynamiques (`$this->tags`, `$this->pagination`,
`$this->show_permissions`…) jamais déclarées sur la classe, un style hérité du
MVC Joomla historique (objets `stdClass`-like) que PHPStan niveau 2 ne peut pas
valider sans déclaration explicite. Une bonne partie de `method.notFound`
(36 lignes) tient au même phénomène côté framework : les stubs Joomla 6 utilisés
par l'analyse statique (`joomla-stubs/`) ne déclarent pas certaines méthodes que
les implémentations concrètes fournissent réellement (`CMSApplicationInterface::
getDocument()/getSession()/getUserState()`, `ComponentInterface::getMVCFactory()/
getContainer()`, `BaseDatabaseModel::getPagination()/getItem()/getForm()`…) —
dette de complétude des stubs plus que bug applicatif, mais qui masque
d'éventuels vrais bugs de type dans le même lot.
...
**Comportement déduit.** La dette de typage est massivement concentrée dans la
couche **View** (les cinq premiers fichiers du tableau, tous des `HtmlView`,
totalisent plus de 130 des 1 639 lignes) — cohérent avec le style de vues Joomla
qui peuplent des propriétés publiques non déclarées consommées ensuite par les
templates `tmpl/*.php`. C'est une zone de fragilité de typage, pas
nécessairement de fragilité fonctionnelle : ces vues sont anciennes et stables
en production (voir §7, aucune régression de View récente dans le CHANGELOG).
...
- **Chantier F — `admin/src/types/*.php`.** Constat : 3 149 lignes utilisant
  ...
  `grep` — voir §5.1), chargées par `require_once` dynamique construit depuis
  une valeur stockée en base, instanciées par `call_user_func`. Le plan
  qualifie explicitement `require_once` construit depuis la base de
  « gadget LFI » potentiel, et le contrat `getNumRecordsQuery()` (qui renvoie
  du SQL brut injecté dans un `SELECT`) de point à remplacer par un entier.
...
- **Chantier H — `eval()` dans les gabarits — risque accepté et documenté.**
  ...
  au rendu frontend. Le plan l'écrit sans détour : « Toute personne disposant
  de `core.edit` sur le composant peut donc exécuter du PHP arbitraire sur le
  serveur — une escalade de privilège que Joomla protège habituellement par
  `core.admin`. » La décision de direction documentée est de conserver
  `eval()`. Une atténuation à coût quasi nul (~0,5 j) est proposée sans être
  planifiée : restreindre l'**écriture** de ces deux champs à `core.admin`
  sans changer le comportement des gabarits existants.
- **Chantier I — dette courte hors refonte**, dont : un `set_error_handler()`
  vide et global qui supprime toutes les notices PHP du site depuis
  `ContentbuilderngImageScale.php:30-31` (0,5 j) ; génération de classeur
  Excel dans un gabarit de vue (`site/tmpl/export/default.php`, violation MVC,
  2 j) ; une requête d'état sans filtre `form_id` dans le même fichier
  (`site/tmpl/export/default.php:149-154`, capable de lire l'état d'un autre
  formulaire, 0,5 j) ; 19 événements `GenericEvent` par chaîne de caractères à
  typer.
- **Chantier J — absence de transactions.** Constat : deux opérations
  multi-étapes non transactionnelles alors qu'elles devraient l'être :
  ...
  un chemin de compensation explicite reste nécessaire même après correction.
...
**Fait observé.** Six sites `eval()` en dehors des tests, tous liés au moteur
de gabarits :
...
| `site/src/Model/EditModel.php:756,763` | `eval($code)` — même famille de risque, côté soumission de formulaire |

**Comportement déduit.** Cette liste corrobore exactement le chantier H du
plan : le risque n'est pas un point isolé mais une famille de trois usages
(parseur de wrapper, `*_prepare` de gabarit détail/édition, et un `eval()`
supplémentaire dans `EditModel.php` côté site) qui partagent la même surface —
du texte configurable en base de données exécuté comme PHP.
...
## 6. Dette de modélisation SQL (`admin/sql/updates/mysql/`)

**Fait observé.** 17 fichiers de migration versionnée, de `6.1.7.sql` à
`6.1.20.sql`. Le premier (`6.1.7.sql`) est explicitement un marqueur de
...
### 6.1 Corrections de schéma a posteriori (dette de modélisation)
...
**Comportement déduit.** Le motif dominant n'est pas la correction de bugs de
type (une seule occurrence, `6.1.10-RC06.sql`), mais l'**extraction tardive de
concepts** initialement confondus avec un champ voisin
(`export_id_column`/`show_id_column`, `list_state_bulk`/`list_state`,
`initial_order_dir2/3` avec un défaut initialement erroné) — signe que le
modèle de données s'est affiné par itération sur des besoins fonctionnels
plutôt que d'avoir été conçu à l'avance pour ces distinctions.
...
**Zone inconnue.** Aucun index ajouté a posteriori n'apparaît dans ces 17
...
(`com_contentbuilderngInstallerScript`, ligne 64), **96 méthodes** (1 publique
de plus le constructeur, le reste très majoritairement `private`).
...
**Comportement déduit.** Le fichier concentre au minimum **onze
responsabilités distinctes** (cycle de vie, logs, permissions fichiers,
nettoyage, auto-réparation de schéma, réparation de menus, gestion de
plugins, reporting de mise à jour, cohérence de version, purge de cache,
migration legacy) dans une classe unique, sans découpage en services
injectables — à l'inverse du reste du composant qui utilise activement
`admin/services/provider.php` et une couche `Service/` pour séparer ces
préoccupations (`MIGRATION.md` confirme que le conteneur DI est en place pour
le runtime). Un script d'installation Joomla doit rester une classe unique
par contrainte du framework (point d'entrée `install()/update()/
uninstall()/postflight()` imposé), mais rien n'empêcherait ces 96 méthodes de
déléguer à des collaborateurs de service testables — ce que le fichier ne
fait pas.
...
**Hypothèse.** La taille de `script.php` (95 Ko, la plus grosse unité de
code du dépôt après agrégation de toutes ses méthodes) est directement liée
à l'historique de migration du composant : `MIGRATION_GUIDE.md` décrit une
migration automatique complète depuis l'ancien `contentbuilder`
(renommage de tables, extensions, assets, menus, thèmes, dédoublonnage — 10
étapes listées) entièrement portée par ce fichier. Le poids du fichier reflète
donc en grande partie la charge de compatibilité ascendante *au niveau
données* que le projet assume malgré la règle « no legacy/deprecated APIs »
d'`AGENTS.md`, laquelle porte sur le code applicatif, pas sur la migration
de schéma/données historique — distinction implicite, non documentée
explicitement comme telle dans `AGENTS.md`.
...
| **Élevé** | `eval()` sur texte configurable en base (`details_prepare`/`editable_prepare`, wrapper d'éléments, `EditModel`) — RCE pour tout titulaire de `core.edit` | `admin/forms/form.xml:127,147` ; `TemplateRenderService.php:1004,1335` ; `EditModel.php:756,763` ; `REFACTORING_PLAN.md` chantier H | Risque accepté par décision de direction documentée ; non planifié |
| **Élevé** | `admin/src/types/*.php` : namespace hors convention, `require_once` dynamique construit depuis la base, `call_user_func` | `REFACTORING_PLAN.md` chantier F ; confirmé §5.1 | Non commencé |
...
| Moyen | Absence de transactions sur 4 opérations DDL/multi-tables | `REFACTORING_PLAN.md` chantier J | Non commencé |
| Moyen | `script.php` : 96 méthodes / 11+ responsabilités dans une classe unique | §8 | Structurel, propre au format script Joomla |
...
| Faible | `set_error_handler()` global vide (supprime toutes les notices du site) | `ContentbuilderngImageScale.php:30-31` ; `REFACTORING_PLAN.md` chantier I | Non planifié, coût annoncé 0,5 j |
| Faible | Requête d'état sans filtre `form_id` dans un gabarit d'export | `site/tmpl/export/default.php:149-154` ; `REFACTORING_PLAN.md` chantier I | Non planifié, coût annoncé 0,5 j |
| Faible | Marqueurs `TODO`/`hack`/`workaround` inline | §3 | Ponctuel, peu nombreux |
| Faible | Dette de modélisation SQL (colonnes extraites a posteriori) | §6 | Corrigée au fil des versions, motif récurrent mais mineur par occurrence |
