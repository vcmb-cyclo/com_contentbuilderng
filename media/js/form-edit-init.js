 ...
 *
 * Relies on window.cbFormEditConfig, populated inline by
 * admin/tmpl/form/edit_init_scripts.php before this file loads.
 */
    ...
    const cbViewportStateKey = 'cbng.form.viewport.' + cbFormEditConfig.formId;
    const cbElementsColumnsStateKey = 'cbng.form.elements.columns.' + cbFormEditConfig.formId;
    const cbPermissionsColumnsStateKey = 'cbng.form.permissions.columns.' + cbFormEditConfig.formId;
    const cbElementsColumnsLabel = cbFormEditConfig.text.columns;
    ...
    const cbFormId = cbFormEditConfig.formId;
    const cbDebugModeEnabled = cbFormEditConfig.debugModeEnabled;
    const cbSaveAnimationDurationMs = 500;
    const cbIsBreezingFormsType = cbFormEditConfig.isBreezingFormsType;
    const cbBreezingFormsEditableToken = cbFormEditConfig.breezingFormsEditableToken;
    const cbEditByTypeEnableConfirm = cbFormEditConfig.text.typeEditEnableBfConfirm;
    const cbFormNotFoundMessage = cbFormEditConfig.text.formNotFound;
    const cbSaveFailedMessage = cbFormEditConfig.text.saveFailed;
    const cbCloseUnsavedMessage = cbFormEditConfig.text.confirmCloseUnsaved;
    const cbUnnamedLabel = cbFormEditConfig.text.unnamed;
    const cbInheritedFromLabel = cbFormEditConfig.text.inheritedFrom;
    ...
    function listItemTask(id, task) {
        ...
        f.limitstart.value = cbFormEditConfig.limitstart;
        ...
        switch (task) {
            ...
            case 'form.apply':
                ...
                if (document.getElementById('name').value == '') {
                    error = true;
                    alert(cbFormEditConfig.text.errorEnterFormname);
                } else if (nodes) {
                    if (typeof nodes.value != 'undefined') {
                        if (nodes.checked && document.adminForm['elementLabels[' + nodes.value + ']'].value == '') {
                            error = true;
                            alert(cbFormEditConfig.text.errorEnterFormnameAll);
                            break;
                        }
                    } else {
    ...
    function cbQueueEditableSampleGeneration(button) {
        ...
        if (cbTemplateHasContent(currentTemplate)) {
            var shouldContinue = confirm(cbFormEditConfig.text.initialiseOverwriteConfirm);
            if (!shouldContinue) {
                return;
            }
        }
